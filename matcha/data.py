...
def fakedata():
	...
	if Fl == 'y':
		...
		for user in users:
			...
			user['latCord'] = round(uniform(-90.000000,90.000000), 7)
			user['lngCord'] = round(uniform(-180.000000,180.000000), 7)
			cur.execute('INSERT INTO users (fname, lname, username, email, password, verify, bio, gender, sexuality, age, latCord, lngCord) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
			[user['name'], user['surname'], user['username'], user['email'], '70d6d3db2b8cee727994e89f9b8c21622e39840ad579dd82da37aadd441473aab9996dd749d652b8023791f3862ca3cc584f9ff9c27222217e77af241d3b3abd54486eeb78c733c57aab7aa7ff5709ec90655dee193c4a32e46ffb2796049d0b', None, user['bio'], user['gender'], user['sexuality'], user['age'], user['latCord'], user['lngCord']])
			i += 1
